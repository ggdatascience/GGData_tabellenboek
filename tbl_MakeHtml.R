  ...
  for (i in 1:nrow(indeling_rijen)) {
    ...
    if (indeling_rijen$type[i] %in% c("titel", "kop", "vraag", "tekst")) { # regel met een titel, kop, of tekst
      ...
      if (is_dich) {
        ...
        table.output = c(table.output, paste0("<h4 class=\"heading vraag\" id=\"heading_", i, "\">", HTMLencode(var_labels$label[var_labels$var == indeling_rijen$inhoud[i] & var_labels$val == "var"]), "</h4>"))
      }
