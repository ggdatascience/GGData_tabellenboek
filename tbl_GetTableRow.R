...
GetTableRow = function (var, design, calculate.ci=T, col.design) {
  ...
  # splitsen op subset?
  # hierbij wordt het even creatief denken: een tabellenboek wordt meestal gemaakt voor een geografische eenheid (gemeente/regio/GGD-gebied)
  ...
  if (any(!is.na(colgroups$subset))) {
    leadingsubset = colgroups$subset[which(!is.na(colgroups$subset))[1]]
    leadingcol = min(col.design$col.index[group_rows(colgroups)[[which(!is.na(colgroups$subset))[1]]]])
  }
  ...
  subsetvals = val_labels(design$variables[[leadingsubset]])
  for (s in 1:length(subsetvals)) {
    ...
    for (i in 1:nrow(colgroups)) {
      ...
      if (colgroups$subset[i] != leadingsubset) {
        subsetval = as.numeric(unique(design$variables[["subregio"]][design$variables[[leadingsubset]] == subsetvals[s]]))
        subsetval = subsetval[!is.na(subsetval)]
        if (length(subsetval) != 1) {
          msg("Let op: aanvullende subset %s heeft %d mogelijkheden in de dataset: %s. Dit is niet mogelijk voor berekening; enkel eerste optie gebruikt.",
              colgroups$subset[i], length(subsetval), str_c(subsetval, collapse=", "), level=WARN)
      ...
      if (!is.na(colgroups$crossing[i]) && !is.na(colgroups$test.col[i]) && colgroups$test.col[i] == 0) {
        ...
        results = bind_rows(results, data.frame(dataset=rep(colgroups$dataset[i], n), subset=rep(colgroups$subset[i], n), subset.val=rep(subsetvals[s], n),
        ...
        if (!is.na(colgroups$test.col[i]) && sum(unweighted, na.rm=T) > 0) {
          ...
          if (!is.na(colgroups$subset[colgroups$test.col[i]])) {
            if (colgroups$subset[colgroups$test.col[i]] == leadingsubset) {
              ...
              # TODO: herkennen wat de nodige corresponderende subset is
              selection = paste0(selection, ".s.", subsetvals[s])
            }
          }
          design.subset = subset(design, eval(parse(text=selection)))
